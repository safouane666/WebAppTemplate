...
- **Routes**: Define API endpoints with Swagger documentation and mount controllers
...
- Hot reload with Nodemon (automatic server restart on file changes)
...
- **API Documentation**: Swagger/OpenAPI automatically generated from route annotations
- **Hot Reload**: Nodemon watches for file changes and restarts the server automatically
...
2. Add Swagger documentation using JSDoc comments above your route handlers
3. Create controller in `packages/backend/src/controllers/`
4. Create service if needed in `packages/backend/src/services/`
5. Mount route in `packages/backend/src/routes/api/index.ts`

**Example with Swagger documentation:**
...
The Swagger documentation will automatically appear at `http://localhost:3001/api-docs` in development mode.
